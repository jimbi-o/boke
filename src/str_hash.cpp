...
namespace boke {
void InitStrHashSystem(AllocatorData* allocator_data) {
  DEBUG_ASSERT(str_hash_database == nullptr, DebugAssert{});
  str_hash_database = CreateStringHashDatabase(allocator_data);
}
void TermStrHashSystem(AllocatorData* allocator_data) {
  Deallocate(str_hash_database, allocator_data);
